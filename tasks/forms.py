from django import forms
from .models import Task

class TaskForm(forms.ModelForm): #crear un formulario de un model
    class Meta:
        model = Task
        fields = ['title','description','important'] #los campos que necesito
        widgets = {#crear vista desde form
            'title': forms.TextInput(attrs={'class':'form-control', 'placeholder':'Escribe el título'}),
            'description': forms.Textarea(attrs={'class':'form-control', 'placeholder':'Escribe una descripción'}),
            'important': forms.CheckboxInput(attrs={'class':'form-check-input m-auto'})
        }